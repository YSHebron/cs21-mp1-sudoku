...
.macro read_int_to(%dest)
	do_syscall(5)
	move	%dest, $v0
.end_macro

# use int:1, float:2, double:3, str:4, char:11
# addu also supports immediates
.macro print_val(%val, %format)
	addu	$a0, $0, %val
	do_syscall(%format)
.end_macro
...
main:
	# TODO: store grid in main stackframe to mimic typical C programs
	# (driver code in main)
	addi	$sp, $sp, -512
	sw	$ra, 508($sp)
	sw	$s0, 0($sp)
